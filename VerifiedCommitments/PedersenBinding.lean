...
lemma binding_as_hard_dlog
  (G : Type) [Fintype G] [Group G] [IsCyclic G] [DecidableEq G] (g : G)
    ...
    haveI : Fact (Nat.Prime q) := ⟨hq_prime⟩;
    Commitment.comp_binding_game (Pedersen.scheme G g q) A 1 ≤ DLog.experiment G g q (DLog.adversary' G q A) 1 := by
  haveI : Fact (Nat.Prime q) := ⟨hq_prime⟩
  -- Unfold definitions
...
theorem computational_binding :
  ∀ (G : Type) [Fintype G] [Group G] [IsCyclic G] [DecidableEq G] (g : G)
    ...
    ∃ hq_prime : Nat.Prime q, Commitment.comp_binding_game (Pedersen.scheme G g q) A 1 ≤ ε) := by
