...
instance {q : ℕ} : Fintype (ZMod q) := sorry
...
/-
  ...
  encrypt, and decrypt
-/
noncomputable def enc_dec  (m : M) : PMF (ZMod 2) :=
do
  let k ← keygen
  let c ← encrypt k.1 m
...
-/
noncomputable def SSG : PMF (ZMod 2):=
do
  let k ← keygen
  let m ← A1 k.1
...
def pke_semantic_security (ε : ENNReal) : Prop :=  (SSG keygen encrypt A1 A2 1) - 1/2 ≤ ε
...
def decrypt (x : ZMod params.q)(c : G × G) : G := (c.2 / (c.1^x.val))
...
-- The lemma states that a * b / a = b in a group
-- This requires showing that division by a on the right cancels multiplication by a on the left
lemma mul_div_cancel_left' (a b : G) : a * b / a = b := by sorry

lemma decrypt_eq_m (m : G) (x y: ZMod params.q) : decrypt x ((params.g^y.val), ((params.g^x.val)^y.val * m)) = m := by
  unfold decrypt
  simp only
  rw [← pow_mul, ← pow_mul, mul_comm x.val y.val]
  exact mul_div_cancel_left' _ _



def elgamal_correctness_prop : Prop :=
  ∀ (m : G), PKE.enc_dec keygen encrypt decrypt m = pure 1


theorem elgamal_correctness : @elgamal_correctness_prop G params := by
  simp [elgamal_correctness_prop]
  intro m
  unfold keygen encrypt decrypt PKE.enc_dec
  ...
  have h : (params.g ^ x.val) ^ y.val * m / (params.g ^ y.val) ^ x.val = m := decrypt_eq_m m x y
  simp [h]
  rfl
...
do
  ...
  let b' ← params.A2 gy (gz * mb) m.2.2
  pure (1 + b + b')
...
-/
theorem SSG_DDH0 : PKE.SSG keygen encrypt params.A1 A2' =  DDH.Game0 G params.g params.q D := by
  simp only [PKE.SSG, DDH.Game0, bind, keygen, encrypt, D]
...
do
  let x ← PMF.uniformOfFintype (ZMod params.q)
  let y ← PMF.uniformOfFintype (ZMod params.q)
  let m ← params.A1 (params.g^x.val)
  let b ← PMF.uniformOfFintype (ZMod 2)
  let ζ ← (do
    let z ← PMF.uniformOfFintype (ZMod params.q)
    let mb ← pure (if b = 0 then m.1 else m.2.1)
    pure (params.g^z.val * mb))
  let b' ← params.A2 (params.g^y.val) ζ m.2.2
  pure (1 + b + b')
...
theorem elgamal_semantic_security (DDH_G : DDH.Assumption G params.g params.q D ε) :
    PKE.pke_semantic_security keygen encrypt params.A1 A2' ε := by
